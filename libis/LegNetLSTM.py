...
class Head_layer(nn.Module):
    ...
    def __init__(self,final_ch ,out_size, 
...
class SeqNN(nn.Module):
    ...
    def __init__(self,
        ...
        self.lstm = nn.LSTM(input_size=self.final_ch, hidden_size=self.final_ch//2,
    ...
    def forward(self, x, predict_score=True):
        ...
        final_states = lstm_out[1][0].transpose(0,1)
        b_s, s_l, h_d = final_states.shape
        final_states = final_states.reshape(b_s, -1)
        logits = self.head(final_states)
        return logits 


class LitLegNetLSTM(pl.LightningModule):
    def __init__(self,
        model_kws: dict = dict(
                    seqsize=40,
                    in_channels=4,
